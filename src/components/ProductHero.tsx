      ...
      <div className="mobile-container relative z-10 pt-20 sm:pt-24 lg:pt-16">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center min-h-[calc(100vh-4rem)] sm:min-h-screen py-8 sm:py-12 lg:py-20">
          ...
          <div className={`order-2 lg:order-1 space-y-6 lg:space-y-8 animate-slide-up text-center px-4 sm:px-0 ${language === 'ar' ? 'lg:text-right' : 'lg:text-left'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
            ...
            {/* Price, Quantity & Buy Button Section */}
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-primary/20 shadow-soft p-6 max-w-2xl mx-auto lg:mx-0">
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 mb-4">
                ...
                {/* Quantity Selector */}
                <div className="flex items-center gap-3 bg-white rounded-full border-2 border-primary/30 px-5 py-2.5">
                  <Button
                    variant="ghost"
                    size="icon"
                ...
                <Link to="/order">
                  <Button 
                    size="lg" 
                    className="bg-gradient-primary hover:scale-105 transition-all duration-300 shadow-elegant px-8 py-4 rounded-full hover:shadow-glow text-lg"
                  >
                    <ShoppingCart className={`w-6 h-6 ${language === 'ar' ? 'ml-2' : 'mr-2'}`} />
                    {t('hero.buy.now')}
                  </Button>
                </Link>
              </div>
              ...
              <div className="flex items-center justify-center lg:justify-start gap-2 text-green-600 font-semibold">
                <CheckCircle className="w-5 h-5" />
                <span>{language === 'ar' ? 'متوفر شحن فوري' : 'Fast Shipping Available'}</span>
              </div>
            </div>
